...
// import { approveUser, rejectUser, getAllUsers } from "../services/apiService";
// import { FaCheckCircle, FaTimesCircle } from "react-icons/fa";
...
//         const { clientId, password, fullName, email } = res.loginDetails;
//         console.log("Sending email with:", { fullName, email, clientId, password });
...
//             client_id: clientId,
//             password: password,
//             message: "Your Grow Capital account has been approved!"
//           },
//           "Qh_FGNNu7iL35ilnH"
//         ).then(
//           result => console.log("Email sent:", result.status),
...
//       {loading && <div className="text-center mb-3"><Spinner animation="border" /> Loading...</div>}
//       {error && <Alert variant="danger">{error}</Alert>}
...
//               <thead>
//                 <tr>
//                   <th>#</th><th>Name</th><th>Email</th><th>User Id</th>
//                   <th>Password</th><th>Mobile</th><th>Date</th><th>Status / Action</th>
//                 </tr>
//               </thead>
//               <tbody>
//                 {users.map((user, index) => (
...
//                           <Button variant="danger" size="sm" className="ms-2" disabled={loading}
//                                   onClick={() => handleStatusChange(user._id, "rejected")}>Reject</Button>
//                         </>
//                       ) : user.status === "rejected" ? (
//                         <>
//                           <span style={{ color: "red", fontWeight: "bold" }}>✘ Rejected</span>
//                           <Button variant="success" size="sm" className="ms-2" disabled={loading}
//                                   onClick={() => handleStatusChange(user._id, "approved")}>Approve</Button>
//                         </>
//                       ) : (
//                         <>
//                           <Button variant="success" size="sm" className="me-2" disabled={loading}
//                                   onClick={() => handleStatusChange(user._id, "approved")}>Approve</Button>
...
//       <Modal show={popup.show} centered onHide={() => setPopup({ ...popup, show: false })}>
//         <Modal.Body className="text-center py-4">
//           {popup.type === "approved" ? <FaCheckCircle size={70} color="green"/> : <FaTimesCircle size={70} color="red"/>}
//           <h4 className="mt-3">{popup.type === "approved" ? "User Approved!" : "User Rejected!"}</h4>
//           <p className="text-muted">{popup.message}</p>
//           <Button variant="primary" className="mt-3" onClick={() => setPopup({ ...popup, show: false })}>OK</Button>
//         </Modal.Body>
//       </Modal>
//     </Container>
//   );
...
const AdminUserManagement = () => {
  ...
  // -------------------------------------------------------------------
  // ⭐ FINAL FIX — ADMIN + USER BOTH GET EMAIL
  // -------------------------------------------------------------------
  const sendEmails = (userInfo) => {
    const { fullName, email, clientId, password } = userInfo;
    ...
    const templateParams = {
      ...
      to_email: email,                // ⭐ user ko email
      admin_email: "stockinvestingbygrowcapital@gmail.com" // ⭐ admin ko email
    };
    ...
    emailjs.send(
      ...
      {
        ...templateParams,
        to_email: "stockinvestingbygrowcapital@gmail.com" // Admin ke email par
      },
      "Qh_FGNNu7iL35ilnH"
    );
  };
    ...
    try {
      ...
      if (newStatus === "approved") {
        ...
        if (!res || !res.loginDetails) {
          console.error("Login details missing in backend response:", res);
          return;
        }

        sendEmails(res.loginDetails); // ⭐ Email call here
      } else if (newStatus === "rejected") {
        res = await rejectUser(userId);
      }
